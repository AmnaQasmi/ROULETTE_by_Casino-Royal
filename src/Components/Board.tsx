...
class Board extends React.Component<any, any> {
  ...
  getRouletteColor = (number: number) => {
    const index = this.rouletteWheenNumbers.indexOf(number);
    const i =
      index >= 0
  ...
  render() {
    const currentItemChips_1_12 = this.props.chipsData.placedChips.get(
      this.other_1_12
    );
    const currentItemChips_2_12 = this.props.chipsData.placedChips.get(
      this.other_2_12
    );
    const currentItemChips_3_12 = this.props.chipsData.placedChips.get(
      this.other_3_12
    );
    const currentItemChips_1_18 = this.props.chipsData.placedChips.get(
      this.other_1_18
    );
    const currentItemChips_even = this.props.chipsData.placedChips.get(
      this.other_even
    );
    const currentItemChips_red = this.props.chipsData.placedChips.get(
      this.other_red
    );
    const currentItemChips_black = this.props.chipsData.placedChips.get(
      this.other_black
    );
    const currentItemChips_odd = this.props.chipsData.placedChips.get(
      this.other_odd
    );
    const currentItemChips_19_36 = this.props.chipsData.placedChips.get(
      this.other_19_36
    );
                ...
                {this.numbers.map((item, index) => {
                  ...
                  return (
                    <tr key={"tr_board_" + index}>
                      {item.map((cell) => {
                        const cellClass = this.getClassNamesFromCellItemType(
                          cell.type,
                          cell.value ?? null
                        );
                        if (
                          ...
                          const tdKey = "td_" + cell.type + "_" + cell.value;
                          const chipKey = "chip_" + cell.type + "_" + cell.value;

                          const currentItemChips = this.props.chipsData.placedChips.get(
                            cell
                          );
                          return (
                            <ChipComponent
                          ...
                          if (cell.value === undefined) {
                            const split = cell.valueSplit + "";
                            chipKeyValue = "split_" + split;
                          }
                          const tdKey = "td_" + cell.type + "_" + chipKeyValue;
                          const chipKey =
